...
class AudioClassifierHelper(
    ...
    fun startAudioClassification() {
        ...
        val lengthMiliSeconds = ((REQUIRE_INPUT_BUFFER_SIZE * 1.0f) / SAMPLING_RATE_IN_HZ) * 1000
        val interval = (lengthMiliSeconds * (1 * overlap)).toLong()
    ...
    private fun classifyAudioAsync(audioRecord: AudioRecord) {
        val audioData = AudioData.create(
            AudioData.AudioDataFormat.create(recorder?.format),
            SAMPLING_RATE_IN_HZ
        )
